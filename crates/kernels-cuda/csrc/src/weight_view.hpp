...
// it, and it never belonged to gemm: the driver names it `ops::WeightView` in
...
//
// The namespace stays `pie_cuda_driver::ops`, so no caller changes. `ops` is a
// leftover from when the directory meant "vendored-wrapper kernels"; renaming
// it is 1099 edits for a nicer word and buys nothing else.
//
