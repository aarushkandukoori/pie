...
| the MB fire itself: `GptOssMbStep` (three-table pso resolution: by-kind plan + shared GEMM lattice + routed slots), device prefill smoke | — | missing — the assembly and its smoke; the tiled/MMA paged sinks, `bind_gptoss_fp16_qmm` and FP16 precast staging stay unclaimed with the shared family's, same deferral same reason |
