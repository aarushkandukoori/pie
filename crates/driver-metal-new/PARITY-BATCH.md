...
| `bind.cpp` (KV staging with per-layer widths) / the step / device smoke | — | missing — gemma's KV region is the one the shared staging cannot describe (per-layer head widths, owner-only allocation, shared readers); the step and smoke follow it |
