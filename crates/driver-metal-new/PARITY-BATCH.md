...
| `kernels.cpp` / `decode_consts.cpp` / `bind.cpp` / `encode.cpp` / device smoke | — | missing — PSO plan, consts (per-layer strides, softcap, PLE params, `norm_plus_one=false`), then the smoke vs the cached gemma-4 checkpoints |
