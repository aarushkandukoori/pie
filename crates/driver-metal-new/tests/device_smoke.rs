...
// The gpt-oss and gemma4 SMOKES are retired with the layers they exercised.
//
// Five tests drove `metal::{GptOss,Gemma4}{Step,MbStep,Engine}` -- the
// per-family steppers and engines -- over reference checkpoints. All five
// were gated on env variables that are not set in CI, so they skipped
// everywhere except a machine somebody had set up by hand.
//
// What runs the same shapes now, unconditionally:
//   - `device_text_fire::gpt_oss_fires_on_the_device_through_the_same_executor`
//   - `device_text_fire::gemmas_side_network_fires_on_the_device`
//   - `device_text_fire::a_mixture_fires_on_the_device_through_the_same_executor`
//   - `device_checkpoint_names` holds both texts to the REAL checkpoints
//     (gpt-oss-20b-MXFP4-Q4, gemma-4-26b, gemma-4-31b): every name the text
//     states is a tensor the load plan publishes, zero misses.
//
// Written down rather than lost: neither text has been held to a checkpoint
// NUMERICALLY -- no argmax, no token sequence. `device_real_weights` does
// that for the llama shape against MLX and nothing does it for these two.
// The 26B and 20B checkpoints are what makes it hard on a 32GB machine.


// The two llama smokes are RETIRED, and what replaced them is stronger.
//
// They decoded mlx_lm's greedy continuation through
// `batch::build_llama_dag` + `metal::LlamaStep` — the per-family layer.
// `tests/device_real_weights.rs::a_generation_agrees_with_mlx_token_for_token`
// decodes the same continuation through the GENERIC executor, and it is a
// stronger claim in two ways: the reference is MLX recomputing the whole
// prefix at every step rather than a recorded list, and the path under test
// is the one that will ship.
//
// The header above still describes what makes llama-3.2-1B the sharp
// exercise — tied embeddings, 64-wide heads, and the llama3 frequency table
// at factor 32, where a wrong table is not a drift but a rope running 32x off
// on the slow dimensions. All three are exercised there.



// The llama ENGINE smokes are retired with the layer they exercised.
//
// `the_llama_engine_decodes_across_fires` and
// `the_llama_engine_isolates_two_requests` drove `metal::LlamaEngine` — the
// per-family engine — across several fires and across two requests.
// `device_real_weights::a_generation_agrees_with_mlx_token_for_token` runs
// the same shape through the generic executor: a prefill and three decodes,
// each reading the KV the last one wrote, held to MLX token for token.
//
// What is NOT yet covered and is written down rather than lost: two requests
// in one fire, isolated from each other. The generation gate runs one.
