...
use driver_cuda_new::launch::KvCacheLayerView;
...
fn records() -> Vec<Record> {
    vec![kernels_cuda::record!(KvCacheLayerView => "::pie_cuda_driver::KvCacheLayerView" {
...
const KV_VIEW_HPP: &str = "attn/kv_cache_view.hpp";
...
fn the_mirrors_have_the_layout_the_cpp_has() {
    let tu = kernels_cuda::abi::emit_layout_assertions(&records(), &[KV_VIEW_HPP]);
...
fn a_wrong_mirror_does_not_compile() {
    let bad = |mutate: &dyn Fn(&mut Record)| {
        let mut rs = records();
        mutate(&mut rs[0]);
        compile(&kernels_cuda::abi::emit_layout_assertions(&rs, &[KV_VIEW_HPP]))
    };
...
fn the_member_count_check_does_not_depend_on_field_names() {
    ...
    let tu = kernels_cuda::abi::emit_layout_assertions(&rs, &[KV_VIEW_HPP])
        .lines()
