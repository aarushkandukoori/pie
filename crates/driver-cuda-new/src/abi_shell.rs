...
/// The shell's KV: one (k, v) pool per layer, plus the capacity in pages.
struct KvState {
    pools: Vec<(crate::cuda::DeviceBuffer, crate::cuda::DeviceBuffer)>,
    num_pages: u32,
}
...
fn step_impl(
    ...
    let plane_bytes = |pages: u32| {
        pages as usize * page_size as usize * kv_heads_i as usize * head_dim_i as usize * 2
    };
    let grow = !matches!(&state.kv, Some(kv) if kv.num_pages >= need_pages);
    if grow {
        let mut pools = Vec::new();
        for _ in 0..model.hf.num_hidden_layers {
            let mut k = alloc.alloc(plane_bytes(need_pages)).map_err(|_| PIE_STATUS_EXHAUSTED)?;
            let mut v = alloc.alloc(plane_bytes(need_pages)).map_err(|_| PIE_STATUS_EXHAUSTED)?;
    ...
    let layers: Vec<crate::launch::KvCacheLayerView> = kv
        .pools
        .iter()
        .enumerate()
        .map(|(i, (k, v))| crate::launch::KvCacheLayerView {
            layer: i as i32,
    ...
    let (ws, decode_plan, prefill_plan) =
        (&mut scratch.ws, &mut scratch.decode_plan, &mut scratch.prefill_plan);
    ...
    let attn = AttnCtx {
        ...
        logits_soft_cap: 0.0,
        sm_scale: 1.0 / (model.hf.head_dim as f32).sqrt(),
    };
...
pub extern "C" fn pie_cuda_encode(
    ...
    if shell(driver).is_none() || encode.is_null() {
        return PIE_STATUS_INVALID_ARGUMENT;
    }
    PIE_STATUS_UNSUPPORTED
}
...
pub extern "C" fn pie_cuda_copy_kv(
    ...
    for (s_id, d_id) in src_pages.iter().zip(dst_pages) {
        ...
        for (l, (k, v)) in kv_ref.pools.iter().enumerate() {
            for (plane, pool) in [k, v].into_iter().enumerate() {
                ...
                let (dst, src, kind) = if host_dst {
                    (
                        ...
                        unsafe { dev.add(*s_id as usize * page_bytes) },
                        cudaMemcpyKind::cudaMemcpyDeviceToHost,
                    )
                } else if host_src {
                    (
                        unsafe { dev.add(*d_id as usize * page_bytes) },
                ...
                let code = unsafe {
                    cudaMemcpyAsync(dst.cast(), src.cast_const().cast(), page_bytes, kind,
    ...
    if !cells.is_empty() {
        ...
        for (i, (k, v)) in kv_ref.pools.iter().enumerate() {
...
pub extern "C" fn pie_cuda_resize_pool(
    ...
    let mut pools = Vec::new();
    for i in 0..model.hf.num_hidden_layers as usize {
        let Ok(mut k) = alloc.alloc(target as usize * page_bytes) else {
            return PIE_STATUS_EXHAUSTED;
        };
        let Ok(mut v) = alloc.alloc(target as usize * page_bytes) else {
            return PIE_STATUS_EXHAUSTED;
        };
        ...
        if let Some(old_kv) = &old {
            let keep = old_kv.num_pages.min(target) as usize * page_bytes;
            let (ok_, ov) = &old_kv.pools[i];
            use cudarc::runtime::sys::{cudaError, cudaMemcpyAsync, cudaMemcpyKind};
