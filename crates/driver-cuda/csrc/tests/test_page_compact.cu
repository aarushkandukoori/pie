...
//
// `kernels::attn::compact_page_csr` is what turns Quest's `attn_page_mask` from an
...
Csr run_device(
    ...
    kernels::attn::compact_page_csr(d.indices, d.indptr, d.last_lens, d.keep, d.counts,
...
void bench(int requests, int pages_per_request) {
    ...
    const int iters = 200;
    for (int i = 0; i < 20; ++i) {
        kernels::attn::compact_page_csr(d.indices, d.indptr, d.last_lens, d.keep,
