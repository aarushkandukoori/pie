...
// is `kernels::sample::argmax_bf16` itself, not a host reimplementation, and the gate is
...
void check_chunked(const char* name, const __nv_bfloat16* d_logits, int rows,
    ...
    for (int base = 0; base < vocab; base += chunk) {
        ...
        kernels::sample::argmax_accumulate_bf16(d_logits + base, rows, width, row_stride,
                                      base, d_acc_val, d_acc_idx, base == 0,
                                      nullptr);
        ++chunks;
    }
    kernels::sample::argmax_finalize_bf16(d_acc_val, d_acc_idx, d_tokens, rows, nullptr);
...
int main() {
    ...
    kernels::sample::argmax_bf16(d_logits, d_ref, kRows, kVocab, nullptr);
    RT(cudaDeviceSynchronize());
    ...
    // launcher must fall back to the scalar path and still agree.
    {
        ...
        RT(cudaMemcpy(d_dense, dense.data(),
                      ...
                      cudaMemcpyHostToDevice));
        kernels::sample::argmax_bf16(d_dense, d_ref, kRows, kNarrowVocab, nullptr);
        RT(cudaDeviceSynchronize());
